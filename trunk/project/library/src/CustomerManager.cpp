...
std::ofstream &operator<<(std::ofstream &output, const CustomerManager &manager) {
    for (const auto &customer : manager.getAll()) {
        output << customer->getName() << "," << customer->getSurname() << "," << std::endl;
    }
    return output;
}
