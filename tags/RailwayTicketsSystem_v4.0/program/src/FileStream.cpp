...
void output::customerRepository(const std::string &path) {
    ...
    for (const auto &customer : CustomerManager::getInstance().getAll()) {
        customerRepositoryStream << customer->getName() << "," << customer->getSurname() << "," << std::endl;
    }
    customerRepositoryStream.close();
}
...
void output::trainRepository(const std::string &path) {
    ...
    for (const auto &train : TrainManager::getInstance().getAll()) {
        trainRepositoryStream << train->getName() << "," << train->getCapacity() << "," << std::endl;
    }
    trainRepositoryStream.close();
}